...
| `top_p` | f64 | Nucleus sampling probability, sent as `top_p` in the request body for OpenAI-compatible, Anthropic, Bedrock and Google providers. Ignored while thinking/reasoning is enabled — Anthropic and OpenAI reject `top_p` there — and never sent to Copilot or over the OpenAI responses path. Only sent when set; the provider's own default applies otherwise. Must be in `(0, 1]` |
